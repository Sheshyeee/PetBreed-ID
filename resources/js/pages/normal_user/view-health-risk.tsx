...
import { Activity, ArrowLeft, CheckCircle2, ChevronRight, Heart, History, Scan as ScanIcon, Shield, TriangleAlert } from 'lucide-react';
import { FC, useMemo } from 'react';
import { PolarAngleAxis, PolarGrid, PolarRadiusAxis, Radar, RadarChart, ResponsiveContainer } from 'recharts';

interface HealthConcern { name: string; risk_level: string; description: string; prevention: string; }
interface Screening { name: string; description: string; }
interface HealthRisksData { concerns: HealthConcern[]; screenings: Screening[]; lifespan: string; care_tips: string[]; }
interface ScanResult { scan_id: string; breed: string; health_risks: string | HealthRisksData; created_at: string; }
interface Props { results: ScanResult; }

const ViewHealthRisk: FC<Props> = ({ results }) => {
    let healthData: HealthRisksData = { concerns: [], screenings: [], lifespan: 'Unknown', care_tips: [] };
    try {
        if (typeof results?.health_risks === 'string') healthData = JSON.parse(results.health_risks);
        else if (typeof results?.health_risks === 'object' && results?.health_risks !== null) healthData = results.health_risks as HealthRisksData;
    } catch (e) { console.error('Failed to parse health risks JSON:', e); }

    const { concerns = [], screenings = [], lifespan = 'Unknown', care_tips = [] } = healthData;

    const getRisk = (risk: string) => {
        const r = risk.toLowerCase();
        if (r.includes('high')) return { badge: 'border-red-200 bg-red-50 text-red-700 dark:border-red-500/25 dark:bg-red-500/[.08] dark:text-red-300', dot: 'bg-red-500 shadow-[0_0_6px_rgba(239,68,68,.7)]', bar: '#ef4444', barBg:'rgba(239,68,68,.12)', score: 80, label: 'High Risk' };
        if (r.includes('moderate')) return { badge: 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-500/25 dark:bg-amber-500/[.08] dark:text-amber-300', dot: 'bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,.7)]', bar: '#f59e0b', barBg:'rgba(245,158,11,.12)', score: 55, label: 'Moderate' };
    ...
    const Panel = ({ icon, title, children, accent = 'emerald' }: { icon: React.ReactNode; title: string; children: React.ReactNode; accent?: 'emerald' | 'pink' | 'cyan' }) => (
        <div className="vhr-panel relative flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/[.07] dark:bg-[#131720]">
            <div className={`flex flex-shrink-0 items-center gap-2 border-b px-4 py-2.5 ${accent === 'pink' ? 'border-pink-100 bg-pink-50/60 dark:border-pink-500/10 dark:bg-pink-500/[.04]' : accent === 'cyan' ? 'border-cyan-100 bg-cyan-50/60 dark:border-cyan-500/10 dark:bg-cyan-500/[.04]' : 'border-slate-200 bg-slate-50/80 dark:border-white/[.06] dark:bg-white/[.025]'}`}>
                <div className={`flex h-5 w-5 items-center justify-center rounded-md border ${accent === 'pink' ? 'border-pink-500/20 bg-pink-500/10 text-pink-600 dark:text-pink-400' : accent === 'cyan' ? 'border-cyan-500/20 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400' : 'border-emerald-500/20 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'}`}>{icon}</div>
                    ...
                    <div className="mx-auto max-w-[1100px] px-4 py-6 sm:px-6 lg:px-4">
                        ...
                        <div className="vhr-fu mb-5 flex flex-wrap items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <Link href="/scan-results" className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 transition-all hover:border-pink-500/30 hover:text-pink-600 dark:border-white/[.07] dark:bg-[#131720] dark:text-slate-400 dark:hover:text-pink-400">
                                    <ArrowLeft size={15} />
                                </Link>
                                <div>
                                    ...
                                    <p className="mt-0.5 text-sm text-slate-600 dark:text-slate-400">Breed-specific health considerations for <strong className="font-semibold text-slate-800 dark:text-slate-200">{results?.breed || 'your dog'}</strong></p>
                                </div>
                            </div>
                        ...
                        <div className="vhr-fu vhr-fu1 mb-5 flex items-start gap-3.5 rounded-2xl border border-red-200 bg-red-50 p-4 dark:border-red-500/20 dark:bg-red-500/[.06]">
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-xs leading-relaxed text-red-700/80 dark:text-red-400/80">This information is for educational purposes only and is not a medical diagnosis. Always consult with a licensed veterinarian for proper medical advice specific to your pet.</p>
                            </div>
                        </div>
                        ...
                        <div className="vhr-fu vhr-fu2 mb-5 grid grid-cols-1 gap-5 lg:grid-cols-[1fr_220px]">
                            ...
                            <Panel icon={<Activity size={11} />} title="Breed Risk Profile">
                                <div className="p-5 sm:p-6">
                                    <div className="mb-4 flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-2.5 dark:border-white/[.05] dark:bg-white/[.025]">
                                        <div className="flex gap-1.5">
                                            <div className="h-2 w-2 rounded-full bg-[#FF5F57]"/><div className="h-2 w-2 rounded-full bg-[#FEBC2E]"/>
                                            <div className="vhr-hudblink h-2 w-2 rounded-full bg-pink-500 shadow-[0_0_5px_#ec4899]"/>
                                        </div>
                                        <span className="vhr-mono text-[9px] text-slate-500 dark:text-slate-500">doglens://health/radar/{results?.scan_id?.slice(0,8)}</span>
                                    </div>
                                    ...
                                    <p className="vhr-mono text-center text-[9px] tracking-[.1em] text-slate-500 dark:text-slate-500">Higher values = more common concerns for this breed</p>
                                </div>
                            </Panel>

                            {/* Lifespan + Care Tips */}
                            <div className="flex flex-col gap-4">
                                <Panel icon={<Heart size={11} />} title="Avg Lifespan" accent="pink">
                                    <div className="flex flex-col items-center justify-center gap-3 p-6">
                                        <div className="relative flex h-24 w-24 items-center justify-center">
                                            ...
                                            <div className="flex h-full w-full flex-col items-center justify-center rounded-full border-4 border-pink-200 bg-pink-50 shadow-lg shadow-pink-500/10 dark:border-pink-500/25 dark:bg-pink-500/[.07]">
                                                <span className="text-2xl font-extrabold text-pink-600 leading-none dark:text-pink-400">{lifespan}</span>
                                                <span className="vhr-mono text-[9px] tracking-[.08em] text-pink-600/70 uppercase dark:text-pink-400/70">yrs</span>
                                            </div>
                                        </div>
                                        <p className="text-[12px] font-semibold text-slate-700 dark:text-slate-300">Average Lifespan</p>
                                    </div>
                                </Panel>
                                        ...
                                        <ul className="flex flex-col gap-1 p-4">
                                            {care_tips.map((tip, i) => (
                                                <li key={i} className="flex items-start gap-2">
                                                    ...
                                                    <span className="text-[11px] leading-relaxed text-slate-700 dark:text-slate-300">{tip}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </Panel>
                        ...
                        <div className="vhr-fu vhr-fu3 mb-5">
                            <div className="mb-3 flex items-center gap-2">
                                <div className="flex h-5 w-5 items-center justify-center rounded-md border border-pink-500/20 bg-pink-500/10 text-pink-600 dark:text-pink-400"><TriangleAlert size={10}/></div>
                                ...
                                <span className="vhr-mono ml-auto rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-[9px] text-slate-600 dark:border-white/[.07] dark:bg-white/[.03] dark:text-slate-400">{concerns.length} identified</span>
                            </div>
                            {concerns.length > 0 ? (
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                    {concerns.map((c, i) => {
                                        ...
                                        return (
                                            <div key={i} className="vhr-card vhr-panel relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/[.07] dark:bg-[#131720]" style={{'--tw-shadow-color':'transparent'} as any}>
                                                {/* Colored top strip */}
                                                <div className="absolute top-0 left-0 right-0 h-[2.5px]" style={{background:`linear-gradient(90deg,transparent,${rc.bar} 30%,${rc.bar} 70%,transparent)`, opacity:.55}}/>
                                                <div className="p-5">
                                                    <div className="mb-3 flex items-start gap-2.5 justify-between">
                                                        ...
                                                        <span className={`vhr-mono flex-shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-bold tracking-[.06em] uppercase ${rc.badge}`}>{c.risk_level}</span>
                                                    </div>
                                                    ...
                                                    <div className="mb-3 space-y-1">
                                                        <p className="vhr-mono text-[9px] tracking-[.1em] text-slate-500 uppercase dark:text-slate-500">Description</p>
                                                    ...
                                                    <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 dark:border-white/[.05] dark:bg-white/[.03]">
                                                        <p className="vhr-mono mb-1 text-[9px] tracking-[.1em] text-slate-500 uppercase dark:text-slate-500">Prevention & Management</p>
                                ...
                                <p className="text-sm text-slate-500 italic dark:text-slate-500">No specific health concerns generated for this scan.</p>
                            )}
                        </div>
                        ...
                        {screenings.length > 0 && (
                            <div className="vhr-fu vhr-fu4">
                                <Panel icon={<Shield size={11} />} title="Recommended Screenings" accent="cyan">
