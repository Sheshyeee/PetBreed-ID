...
export default function Scan() {
    ...
    const startCamera = async () => {
        if (!isCameraOk()) { alert('Camera available on Chrome, Edge, Safari, Firefox.'); return; }
        try {
            setCameraError(null);
    ...
    const switchCamera = async () => {
        ...
        setTimeout(async () => {
            try {
                const ms = await navigator.mediaDevices.getUserMedia({
                    video: { facingMode: nm, width: { ideal: 1920 }, height: { ideal: 1080 } },
                    audio: false,
                });
                setStream(ms);
                if (videoRef.current && ms.active) {
    ...
    const capturePhoto = () => {
        ...
        c.toBlob(
            (b) => { if (b) processFile(new File([b], `capture-${Date.now()}.jpg`, { type: 'image/jpeg' })); },
            'image/jpeg', 0.95,
        );
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!data.image) { alert('Select an image first'); return; }
        setShowLoading(true);
        post('/analyze', {
            forceFormData: true,
    ...
    const TopBreedsContent = () => (
        <div className="flex flex-col gap-0.5 p-2.5">
            {topBreeds.length === 0 ? (
                <p className="py-3 text-center font-mono text-[10px] text-slate-400 dark:text-slate-600">No scan data yet</p>
            ) : (
                topBreeds.map((b, i) => (
                    <div key={i} className="group flex cursor-default items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-slate-50 dark:hover:bg-white/[.03]">
                        <span className="w-4 flex-shrink-0 font-mono text-[9px] text-slate-500 group-hover:text-emerald-500/70 dark:text-slate-600">#{i + 1}</span>
                        <span className="flex-1 truncate text-[12px] font-medium text-slate-700 dark:text-slate-300" title={b.breed}>{b.breed}</span>
                        <div className="h-[3px] w-10 flex-shrink-0 overflow-hidden rounded-full bg-slate-200 dark:bg-white/[.07]">
                            <div className="h-full rounded-full bg-emerald-500/60" style={{ width: `${b.bar_width}%` }} />
                        </div>
                    </div>
                ))
            )}
    ...
    const AppointmentBanner = () => {
        ...
        return (
            <Link
                href="/appointments"
                className="bnr group relative flex items-stretch overflow-hidden rounded-2xl no-underline"
                style={{ boxShadow: '0 4px 24px rgba(99,102,241,.18), 0 1px 4px rgba(0,0,0,.08)' }}
            >
                ...
                <div className="flex flex-1 items-center justify-between gap-4 border border-l-0 border-indigo-200/70 bg-indigo-50 px-4 py-3 transition-colors group-hover:bg-indigo-100/70 dark:border-indigo-400/15 dark:bg-[#1a1f35] dark:group-hover:bg-indigo-400/[.08]">
                    <div className="min-w-0 flex-1">
                        <div className="mb-1 flex items-center gap-2">
                            ...
                            {count > 1 && (
                                <span className="rounded-full bg-indigo-200 px-1.5 py-0.5 font-mono text-[8px] font-bold text-indigo-700 dark:bg-indigo-400/20 dark:text-indigo-300">
                                    +{count - 1} more
                                </span>
                            )}
                        </div>
                        ...
                        <p className="mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-0.5 font-mono text-[10px] text-slate-500 dark:text-slate-400">
                            ...
                            <span className="text-slate-300 dark:text-white/20">·</span>
                            <span className="inline-flex items-center gap-1">
                                <Stethoscope size={9} className="text-violet-400" />
                                {next.vet_name}
                            </span>
                        </p>
                    </div>
                    <div className="bnr-cta flex flex-shrink-0 items-center gap-1.5 rounded-xl bg-indigo-600 px-3.5 py-2.5 text-[12px] font-bold text-white"
                        ...
                        Respond
                        <ArrowUpRight size={13} className="bnr-arrow" />
                    </div>
                </div>
            </Link>
        );
    ...
    return (
        <>
            <style>{`
                ...
                @keyframes bnr-in     { from{opacity:0;transform:translateY(-5px) scale(.99)} to{opacity:1;transform:translateY(0) scale(1)} }
                @keyframes bnr-ping   { 0%{transform:scale(1);opacity:1} 70%,100%{transform:scale(2.4);opacity:0} }
                ...
                .bnr-icon::before {
                    ...
                    transform:skewX(-14deg); pointer-events:none; opacity:0; transition:opacity .15s;
                }
                ...
                .bnr-arrow { transition: transform .18s; }
            ...
            <div className="sc-root flex h-screen flex-col overflow-hidden bg-slate-50 transition-colors duration-300 dark:bg-[#080B0F]">
                ...
                {/* ── 2. Banner (flex-shrink-0 → takes own height, shrinks flex-1 below) ── */}
                <div className="relative z-20 flex-shrink-0 px-4 py-2">
                    <AppointmentBanner />
                </div>
                ...
                {/* QR Modal */}
                {showQRModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-xl" onClick={() => setShowQRModal(false)}>
                        <div className="sc-modalup sc-panel-line relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-7 shadow-2xl dark:border-white/[.08] dark:bg-[#131720]" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => setShowQRModal(false)} className="absolute top-3 right-3 flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-white/[.06] dark:hover:bg-white/10">
                                <X size={13} />
                            </button>
                            <div className="mb-5 text-center">
                                ...
                                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Install Mobile App</h2>
                                <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Scan QR to download the Android app</p>
                            </div>
                            ...
                            <div className="mb-4 overflow-hidden rounded-xl border border-slate-200 dark:border-white/[.07]">
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-2.5 border-b border-slate-100 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-700 last:border-none dark:border-white/[.05] dark:bg-white/[.02] dark:text-slate-400">
                                        <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md bg-emerald-500/10 text-emerald-500">{f.icon}</div>
                                        {f.t}
                                    </div>
                                ))}
                            </div>
                            <button onClick={() => setShowQRModal(false)} className="w-full rounded-xl bg-emerald-500 py-3 text-sm font-bold text-black hover:bg-emerald-400">Close</button>
                        </div>
                    </div>
                )}

                <button onClick={() => setShowQRModal(true)} className="sc-fab fixed right-5 bottom-5 z-40 flex h-11 w-11 items-center justify-center rounded-[13px] bg-emerald-500 text-black shadow-xl shadow-emerald-500/25 transition-all hover:scale-105 hover:bg-emerald-400 active:scale-95">
                    <QrCode size={17} />
                </button>
                        ...
                        <div className="flex flex-col gap-3 p-3 pb-24 lg:grid lg:grid-cols-[210px_1fr_220px] lg:gap-4 lg:p-4 lg:pb-16 xl:grid-cols-[224px_1fr_232px]">
                            ...
                            <div className="sc-fu sc-nsb flex flex-col gap-3 lg:py-4">
                                ...
                                <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-3">
                                    <div>
                                        <div className="mb-1.5 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/[.08] px-2.5 py-1">
                                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_6px_#10b981]" style={{ animation: 'sc-dpulse 2s ease-in-out infinite' }} />
                                            <span className="sc-mono text-[10px] font-semibold tracking-[.12em] text-emerald-700 uppercase dark:text-emerald-400">Breed Detection</span>
                                        </div>
                                        <h1 className="text-lg leading-none font-extrabold tracking-tight text-slate-900 sm:text-xl dark:text-white">Scan Your Dog</h1>
                                        <p className="mt-1 text-sm leading-relaxed text-slate-600 dark:text-slate-400">Upload a photo or use your camera to identify your dog's breed.</p>
                                    </div>
                                    <Link href="/scanhistory" className="inline-flex flex-shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-[13px] font-semibold text-slate-600 no-underline transition-all hover:border-emerald-500/30 hover:bg-emerald-500/[.03] hover:text-emerald-700 dark:border-white/[.07] dark:bg-[#131720] dark:text-slate-400 dark:hover:text-emerald-400">
                                        <History size={14} />Scan History<ChevronRight size={12} className="opacity-35" />
                                    </Link>
                                </div>
                                ...
                                {localError && showLocalError && (
                                    <div className="sc-alertin flex-shrink-0 rounded-2xl border border-red-200 bg-red-50 p-3.5 dark:border-red-500/20 dark:bg-red-500/[.07]">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[13px] font-bold text-red-600 dark:text-red-400">{localError.not_a_dog ? 'Not a Dog ' : 'Analysis Error'}</p>
                                                <p className="mt-0.5 text-xs text-red-600/80 dark:text-red-500/75">{localError.message}</p>
                                                {localError.not_a_dog && (
                                                    <button onClick={handleReset} className="mt-2 inline-flex gap-1.5 rounded-lg bg-red-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-400">Try Another Image</button>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="sc-maincard relative flex flex-shrink-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/[.07] dark:bg-[#131720]">
                                    <div className="flex flex-shrink-0 items-center gap-3 border-b border-slate-200 bg-slate-50 px-4 py-2.5 dark:border-white/[.06] dark:bg-[#0D1117]">
                                        ...
                                        <div className="sc-mono ml-auto flex items-center gap-1.5 text-[10px] text-emerald-600 select-none dark:text-emerald-400">
                                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_5px_#10b981]" style={{ animation: 'sc-dpulse 2s infinite' }} />
                                            ...
                                            <div className="flex flex-col gap-3">
                                                ...
                                                >
                                                    ...
                                                    <div className="relative flex h-16 w-16 flex-shrink-0 items-center justify-center">
                                                        <div className="sc-ring1" /><div className="sc-ring2" />
                                                    ...
                                                    <div className="text-center select-none">
                                                        ...
                                                        <p className="mt-1 text-[13px] text-slate-500 dark:text-slate-500">or <span className="font-bold text-emerald-600 dark:text-emerald-400">click to browse</span></p>
                                                    </div>
                                                ...
                                                <button type="button" onClick={startCamera} className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-[13px] font-bold text-emerald-700 transition-all hover:border-emerald-500/40 hover:bg-emerald-500/[.05] dark:border-emerald-500/25 dark:bg-white/[.03] dark:text-emerald-400">
                                        ...
                                        {showCamera && !preview && (
                                            <div className="flex flex-col gap-4">
                                                <div className="relative overflow-hidden rounded-2xl border border-emerald-500/25 shadow-md shadow-emerald-500/5">
                                                    <video ref={videoRef} autoPlay playsInline muted className="block w-full bg-black object-cover" style={{ maxHeight: '55vh', minHeight: 240 }} />
                                                    <canvas ref={canvasRef} className="hidden" />
                                                    <div className="pointer-events-none absolute inset-0 z-[3]">
                                                        <div className="sc-cambeam" />
                                                        {['tl','tr','bl','br'].map((p) => <div key={p} className={`sc-hc sc-h${p}`} />)}
                                                    ...
                                                    <button type="button" onClick={switchCamera} className="absolute top-3 right-3 z-10 flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-black/55 text-white backdrop-blur-sm transition-all hover:border-emerald-500/40 hover:bg-emerald-500/20">
                                                        <SwitchCamera size={15} />
                                                    </button>
                                                </div>
                                                <div className="flex gap-3">
                                                    <button type="button" onClick={capturePhoto} className="sc-shim flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 py-3 text-[13px] font-bold text-black shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
                                                        <ScanIcon size={15} /> Capture & Scan
                                                    </button>
                                                    <button type="button" onClick={stopCamera} className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-100 px-4 py-3 text-[13px] font-semibold text-slate-600 transition-all hover:bg-slate-200 dark:border-white/[.08] dark:bg-white/[.05] dark:text-slate-400 dark:hover:bg-white/10">
                                                        <X size={13} /> Cancel
                                                    </button>
                                                </div>
                                            </div>
                                                    ...
                                                    {particleActive && (
                                                        <div className="pointer-events-none absolute inset-0 z-10 overflow-hidden">
                                                            {[...Array(12)].map((_, i) => {
                                                                ...
                                                                return <div key={i} className="sc-pt" style={{ '--dx': `${Math.cos((a*Math.PI)/180)*d}px`, '--dy': `${Math.sin((a*Math.PI)/180)*d}px` } as any} />;
                                                            })}
                                                        </div>
                                                    )}
                                                    <img src={preview} alt="Preview" className="block w-full bg-slate-100 object-contain dark:bg-[#0D1117]" style={{ maxHeight: 360 }} />
                                                    <div className="pointer-events-none absolute inset-0">
                                                        <div className="sc-imglines" /><div className="sc-imgsweep" />
                                                        {['tl','tr','bl','br'].map((p) => <div key={p} className={`sc-hc sc-h${p}`} style={{ opacity: 0.9 }} />)}
                                                        <div className="absolute right-2.5 bottom-2.5 left-2.5 z-[5] flex items-center justify-between">
                                                            ...
                                                            {fileInfo && <span className="sc-mono max-w-[180px] truncate rounded border border-emerald-500/15 bg-black/65 px-2 py-0.5 text-[8px] text-emerald-400 backdrop-blur-sm">{fileInfo.split('·').slice(1).join('·').trim()}</span>}
                                                        </div>
                                                    </div>
                                                </div>
                                                {fileInfo && <p className="sc-mono truncate text-center text-[9px] tracking-wide text-slate-500 dark:text-slate-600">{fileInfo}</p>}
                                                <div className="flex gap-3">
                                                    <button type="submit" disabled={processing} className="sc-shim flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-500 py-3 text-[13px] font-bold text-black shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5 hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-40">
                                                        <ScanIcon size={15} /> {processing ? 'Analyzing…' : 'Analyze Image'}
                                                    </button>
                                                    <button type="button" onClick={handleReset} disabled={processing} className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-100 px-4 py-3 text-[13px] font-semibold text-slate-600 transition-all hover:bg-slate-200 disabled:opacity-40 dark:border-white/[.08] dark:bg-white/[.05] dark:text-slate-400 dark:hover:bg-white/10">
                                                        <X size={13} /> Reset
                                                    </button>
                                                </div>
                                            </div>
                                ...
                                <div className="flex flex-col gap-3 lg:hidden">
                                    <Panel icon={<TrendingUp size={11} />} title="Top Breeds"><TopBreedsContent /></Panel>
                                    <Panel icon={<Activity size={11} />} title="Global Stats"><GlobalStatsContent /></Panel>
                                </div>
                            </div>
