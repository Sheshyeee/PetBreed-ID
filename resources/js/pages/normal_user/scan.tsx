...
export default function Scan() {
    ...
    useEffect(() => {
        if (cameraError) { const t = setTimeout(() => setCameraError(null), 5000); return () => clearTimeout(t); }
    }, [cameraError]);
    ...
    const startCamera = async () => {
        ...
        try {
            ...
            const msgs: Record<string, string> = { NotAllowedError: 'Allow camera permissions.', NotFoundError: 'No camera found.', NotReadableError: 'Camera in use.' };
    ...
    const switchCamera = async () => {
        ...
        setTimeout(async () => {
            ...
            } catch { setCameraError('Failed to switch camera.'); setFacingMode((f) => (f === 'user' ? 'environment' : 'user')); }
        }, 200);
    };
    ...
    const capturePhoto = () => {
        ...
        c.toBlob((b) => { if (b) processFile(new File([b], `capture-${Date.now()}.jpg`, { type: 'image/jpeg' })); }, 'image/jpeg', 0.95);
    };
            ...
            {topBreeds.length === 0 ? (
                ...
                topBreeds.map((b, i) => (
                    <div key={i} className="group flex cursor-default items-center gap-2.5 rounded-lg px-2 py-1.5 transition-colors hover:bg-slate-50 dark:hover:bg-white/[.03]">
                        ...
                        <span className="flex-1 truncate text-[12px] font-medium text-slate-700 dark:text-slate-300" title={b.breed}>
                            {b.breed}
                        </span>
    ...
    return (
        <>
            ...
            <div className="sc-root flex h-screen flex-col overflow-hidden bg-slate-50 transition-colors duration-300 dark:bg-[#080B0F]">
                ...
                <div className="relative z-30"><AnalysisLoadingDialog isOpen={showLoading} /></div>
                        ...
                        <div className="sc-modalup sc-panel-line relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-7 shadow-2xl dark:border-white/[.08] dark:bg-[#131720]" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="mb-5 flex justify-center">
                                <div className="rounded-xl bg-white p-2.5 shadow-lg">
                                    <img src="/doglens_apk_qr.jpeg" alt="QR" className="block h-32 w-32" />
                                </div>
                            </div>
                            ...
                            <div className="sc-fu flex min-h-0 flex-col gap-3">
                                ...
                                <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <h1 className="text-lg leading-none font-extrabold tracking-tight text-slate-900 sm:text-3xl dark:text-white">Scan Your Dog</h1>
                                        {/* FIXED: was text-slate-500 → text-slate-600 */}
                                    ...
                                    <div className="sc-alertin flex-shrink-0 rounded-2xl border border-red-200 bg-red-50 p-3.5 dark:border-red-500/20 dark:bg-red-500/[.07]">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-[13px] font-bold text-red-600 dark:text-red-400">{localError.not_a_dog ? 'Not a Dog Detected' : 'Analysis Error'}</p>
                                        ...
                                        {!preview && !showCamera && (
                                            <div className="flex flex-col gap-3">
                                                ...
                                                >
                                                    ...
                                                    <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e: ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) processFile(f); }} />
                                                ...
                                                <div className="relative overflow-hidden rounded-2xl border border-emerald-500/25 shadow-md shadow-emerald-500/5">
                                                    ...
                                                    <div className="pointer-events-none absolute inset-0 z-[3]">
                                                        ...
                                                        <div className="absolute top-1/2 left-1/2 h-12 w-12 -translate-x-1/2 -translate-y-1/2">
                                                            <div className="sc-camh" /><div className="sc-camv" /><div className="sc-camdot" />
                                                        </div>
                                ...
                                <Panel icon={<Eye size={11} />} title="How It Works">
                                    <div className="flex flex-col p-3">
                                        {[
                                            ...
                                            { n: '02', t: 'AI model analyzes visual breed features in ~1.2s' },
                                    ...
                                    <div className="flex flex-col p-3">
                                        {captureTips.map((tip, i) => (
                                            <div key={i} className={`flex items-start gap-2.5 py-2 ${i < captureTips.length - 1 ? 'border-b border-slate-200 dark:border-white/[.05]' : ''}`}>
                                                ...
                                                <p className={`text-[12px] leading-relaxed ${tip.bold ? 'font-bold text-slate-800 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}>{tip.text}</p>
                                            </div>
                                        ))}
                                    </div>
                                </Panel>
