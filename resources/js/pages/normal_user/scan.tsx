...
export default function Scan() {
    ...
    const Panel = ({
        ...
        <div className="sc-panel relative flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/[.07] dark:bg-[#131720]">
    ...
    const NavLinks = () => (
        <div className="flex flex-col gap-1 p-2.5">
            <Link
                href="/scan"
                className="flex items-center gap-2.5 rounded-xl border border-emerald-500/25 bg-emerald-500/[.09] px-3 py-2.5 text-[13px] font-semibold text-emerald-600 no-underline dark:bg-emerald-500/[.11] dark:text-emerald-400"
            >
                <ScanIcon size={13} />
                <span>New Scan</span>
        ...
        <div className="flex flex-col gap-0.5 p-2.5">
            {topBreeds.length === 0 ? (
                ...
                topBreeds.map((b, i) => (
                    ...
                    >
                        <span className="sc-mono w-4 flex-shrink-0 text-[9px] text-slate-300 group-hover:text-emerald-500/50 dark:text-slate-600">
                            #{i + 1}
                        </span>
                        <span
    ...
    return (
        <>
            <style>{`
                ...
                /* Mobile: single column, everything stacks naturally in DOM order */
                .sc-page {
                    display: flex;
                ...
                /* Desktop: 3-col grid, no scroll on outer wrapper */
                ...
                @media (min-width: 1280px) {
                    .sc-page { grid-template-columns: 224px 1fr 232px; }
                }
                ...
                @media (min-width: 1024px) { .sc-left { display: flex; } }

                /* Center — fills available space on desktop */
                .sc-center {
                    grid-area: center;
                    display: flex;
                    flex-direction: column;
                ...
                /* Right sidebar — bottom of page on mobile, right col on desktop */
                .sc-right {
                    grid-area: right;
                    display: flex;
                    flex-direction: column;
                ...
                @media (min-width: 1024px) {
                    .sc-right { overflow-y: auto; overflow-x: hidden; }
                }

                /* Mobile-only panels (duplicated data, shown only on mobile) */
                .sc-mobile-only { display: block; }
                @media (min-width: 1024px) { .sc-mobile-only { display: none; } }

                /* Scan card — fixed height on mobile, flex fill on desktop */
                .sc-card-shell {
                    position: relative;
                    display: flex;
                    flex-direction: column;
                ...
                @media (min-width: 1024px) {
                    .sc-card-shell { flex: 1; min-height: 0; }
                }
                .dark .sc-card-shell { border-color: rgba(255,255,255,.07); background: #131720; box-shadow: none; }

                /* Form fills card */
                .sc-form-body {
                    display: flex; flex-direction: column;
                    height: 100%; padding: 18px; gap: 14px;
                    overflow-y: auto;
                }
                .sc-dropzone-wrap {
                    display: flex; flex-direction: column;
                ...
                .sc-dz-grow { flex: 1; min-height: 160px; }
            `}</style>
            ...
            <div className="sc-root flex h-screen flex-col overflow-hidden bg-slate-50 transition-colors duration-300 dark:bg-[#080B0F]">
                ...
                {showQRModal && (
                    ...
                    >
                        ...
                        >
                            <button
                                onClick={() => setShowQRModal(false)}
                                className="absolute top-3 right-3 flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-400 hover:bg-slate-200 dark:bg-white/[.06] dark:hover:bg-white/10"
                            >
                                <X size={13} />
                            </button>
                ...
                <div className="relative z-10 min-h-0 flex-1 overflow-hidden">
                    <div className="sc-nsb sc-page mx-auto h-full max-w-[1360px]">
                        ...
                        {/* ── CENTER ──
                             DOM ORDER determines mobile stack:
                             1. Navigation (mobile-only)
                             2. Title + scan card  ← this is the CENTER div
                             3. Top Breeds (mobile-only)
                             4. Global Stats (mobile-only)
                             5. Right sidebar (How It Works, Capture Tips)

                             To achieve this, we put mobile-only panels INSIDE sc-center
                             but only render them visually on mobile.
                        ── */}
                        <div className="sc-center sc-fu">
                            {/* 1. NAVIGATION — mobile only, top of stack */}
                            <div className="sc-mobile-only flex-shrink-0">
                                <Panel
                                    icon={<ScanIcon size={11} />}
                                    title="Navigation"
                                >
                            ...
                            {/* 3. SCAN CARD */}
                            <div className="sc-card-shell sc-maincard">
                                {/* Terminal bar */}
                                ...
                                <form
                                    ...
                                    className="sc-form-body"
                                >
                                    {/* STATE A: Drop zone */}
                                    {!preview && !showCamera && (
                                        <div className="sc-dropzone-wrap">
                                            <div
                                                className={`sc-dz sc-dz-grow flex cursor-pointer flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed px-6 transition-all ${isDragging ? 'sc-dz-on border-emerald-500 bg-emerald-500/[.04] dark:bg-emerald-500/[.06]' : 'border-slate-200 hover:border-emerald-400 hover:bg-emerald-500/[.02] dark:border-white/[.09] dark:hover:border-emerald-500/50'}`}
                                                onClick={() =>
